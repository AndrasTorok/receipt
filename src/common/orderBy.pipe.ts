...
export class OrderByPipe implements PipeTransform {
    transform<T>(array: Array<any>, orderField: string, isAscending: boolean): T[] {
        ...
        array.sort((firstObject: any, secondObject: any) => {
            let firstValue = firstObject[orderField], secondValue = secondObject[orderField];                            

            if (firstValue == undefined && secondValue == undefined) return 0;
            if (firstValue == undefined && secondValue != undefined) return isAscending ? 1 : -1;
            if (firstValue != undefined && secondValue == undefined) return isAscending ? -1 : 1;
            if (firstValue == secondValue) return 0;

            let isString = typeof(firstValue) == 'string' || typeof(secondValue) == 'string';
