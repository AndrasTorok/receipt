...
export class TreatmentItem extends CommonEntity<TreatmentItem> {
    static validityMap = new Map<string, IValidity<TreatmentItem>[]>([
        ...
        ['EndDay',
            [{
                ...
                message: (entity: TreatmentItem) =>
                    `Pana in ziua de aplicare trebuie sa fie mai mare decat ziua din care se incepe aplicarea.`
            },
            {
    ...
    constructor(treatmentItemOrId: ITreatmentItem | number) {
        ...
        if (Number.isInteger(<number>treatmentItemOrId)) {
            ...
            treatmentItem = <ITreatmentItem>treatmentItemOrId;
        }

        for (const prop in treatmentItem) {
            if (treatmentItem.hasOwnProperty(prop)) {
